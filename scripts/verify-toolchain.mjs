...
assert.match(buildWorkflow, /RUNNER_CIDR="\$\{RUNNER_IP\}\/32"/);
assert.match(buildWorkflow, /Temporary TCR public ACL required/);
assert.match(buildWorkflow, /Do not use the all-public shortcut/);
assert.match(buildWorkflow, /DEADLINE=\$\(\(SECONDS \+ 900\)\)/);
assert.match(buildWorkflow, /Remind operator to remove temporary TCR ACL/);
